...
`contacts:submit` - событие подтверждает когда была нажата кнопка подтвеждения отправки в форме для контактов 
`contacts:email` - событие сообщает когда пользователь вводит email в форму
`contacts:phone` - событие сообщает когда пользователь вводит номер телефона в форму

### Order

`order:submit` - событие соощает о нажатии подтверждения отправки формы с данными для покупки
`order:address` - событие сообщает когда пользователь вводит адрес в форму
`order:payment` - событие сообщает когда пользователь выбирает один из представленных типов оплаты
### BasketList

`basket:order` - событие сообщает о нажатии кнопки подтверждения в списке выбранных пользователем товаров

### Header 

`basket:render` - событие сообщает, что необходимо отобразить коризну с выбранными товарами, при нажатии на значок корзины 

### Modal 

`modal:close` - событие сообщает, что необходимо закрыть модальное окно при нажатии на кнопку закрытия окна (крестик)

### Success 

`success:close` - событие сообщает о нажатии на кнопку "за новыми покупками" в окне с подтвеждением покупки
...
`basket:change` - обрабатывает любые изменения в коризне и меняет значение счётчика в заголовке если число товаров изменилось и рендерит корзину

`basket:оpen` - открывает коризну при нажатии на её значок в заголовке.
Открывает модальное окно с корзиной

`basket:deleteItem` - удаляет из модели корзины элемент. закрывает модальное окно чтобы была возможность его перерисовать
